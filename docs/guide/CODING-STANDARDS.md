...
## 6. Struktur Modul & Subpaket Sub-Fitur

Ketika sebuah modul domain (`backend/internal/modules/<domain>/`) memiliki **sub-fitur dengan boundary yang jelas** — sumber data, perilaku, atau dependensi eksternal yang berbeda dari CRUD inti — sub-fitur tersebut **wajib dipisah menjadi subpaket tersendiri**, bukan ditumpuk sebagai file tambahan di root modul.

Contoh resmi: katalog institusi eksternal (sumber: Public API API Indonesia) dipisah ke `backend/internal/modules/people/institutions/` (service + controller + mapper), sementara modul induk `people` tetap murni menangani CRUD internal, relasi `interns`, dan lifecycle.

### 6.1 Kapan Subpaket Wajib Dibuat
Buat subpaket `backend/internal/modules/<domain>/<subfitur>/` apabila sub-fitur memenuhi minimal dua kriteria berikut:
1. Memiliki sumber data atau dependensi eksternal tersendiri (API pihak ketiga, worker khusus, protokol khusus).
2. Memiliki service dan controller tersendiri (minimal 3 file: `service.go`, `controller.go`, `mapper.go`/pendukung).
3. Logikanya dapat dipahami, diuji, dan diubah tanpa membaca kode inti modul induk.

### 6.2 Isi Subpaket & Batas Tanggung Jawab
* **Subpaket menyimpan:** service, controller, mapper/adapter, DTO request, dan test milik sub-fitur.
* **Modul induk menyimpan:** model domain, konstanta kolom/tabel, repository inti, CRUD internal, dan relasi antar-entitas.
* Tipe bersama (struct domain, konstanta status/kolom) **tetap di modul induk**; subpaket hanya mendefinisikan tipe miliknya sendiri (DTO eksternal, hasil pencarian, request impor).

### 6.3 Arah Dependensi (Anti Import-Cycle)
* Subpaket **boleh** mengimpor modul induknya (untuk tipe domain dan repository yang sudah `exported`).
* Modul induk **dilarang** mengimpor subpaketnya — pelanggaran ini menyebabkan *import cycle* dan gagal `go build`.
* Verifikasi setiap perubahan struktur dengan `go build ./...`.

### 6.4 Wiring & Routing
* Instansiasi service dan controller subpaket dilakukan terpusat di `backend/cmd/api/main.go`, sejajar dengan modul lain.
* Route sub-fitur menggunakan controller subpaket; URL tetap berada di bawah prefix domain induk (contoh: `/api/v1/people/institutions/search-external` ditangani `institutionsCtrl`, bukan `peopleCtrl`).

### 6.5 Pengecualian: Integrasi Cross-Cutting
Apabila integrasi eksternal dipakai oleh **lebih dari satu domain**, jangan taruh di subpaket modul — taruh di `backend/internal/integrations/` sebagai paket provider mandiri dengan registry generik (`Register`/`Get`/`Names` via interface `Provider`), lalu konsumsi dari subpaket/modul yang membutuhkan. Contoh: `internal/integrations/apiindonesia` dikonsumsi oleh `people/institutions`.

### 6.6 Checklist Subpaket
- [x] Sub-fitur berada di `backend/internal/modules/<domain>/<subfitur>/`, bukan file lepas di root modul.
- [x] Tidak ada import dari modul induk ke subpaketnya (`go build ./...` lolos).
- [x] Wiring dan route terdaftar di `cmd/api/main.go`.
- [x] Subpaket memiliki test tersendiri (mock server untuk dependensi eksternal).
