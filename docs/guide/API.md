...
```
* **Catatan sanitasi (F-ERR-01):** envelope aktual hanya memuat field `success`,
  `statusCode`, `message`, `data?`, `errors?`, `meta?` — **tanpa** field `error`
  maupun `traceId`. Pesan validasi statis yang aman diteruskan apa adanya,
  sedangkan teks yang mengandung detail driver database (SQLSTATE, nama
  tabel/kolom, constraint) diganti pesan generik berbahasa Indonesia; detail
  teknis hanya tercatat di log sisi server.

### 1.4 Authentication Mechanism
* **User Accounts:** Stateless JWT Bearer Token in `Authorization: Bearer <access_token>` header.
  - Access Token TTL: **15 minutes** (contains `user_id`, `email`, `role`, `scopes`, `type: access`).
  - Refresh Token TTL: **7 days** (contains `type: refresh` + `jti` unik; hash SHA-256-nya
    disimpan di tabel `refresh_sessions` untuk rotasi dan revocation).
  - **Refresh Rotation:** setiap `POST /auth/refresh` yang valid menerbitkan pasangan
    token baru dan mencabut sesi lama seketika; token yang sudah diputasi,
    kedaluwarsa, dicabut, atau milik pengguna nonaktif ditolak `401`.
  - **Logout:** `POST /auth/logout` (terproteksi) mencabut sesi refresh perangkat saat ini.
* **Device Gateway (Hardware Terminals):** Header-based authentication (F-AUTH-01):
  - `X-Device-ID: <terminal_identifier>` (string, contoh `GATE-ESP32-01` — bukan UUID)
  - `X-Timestamp: <Unix Timestamp>` (jendela anti-replay ±60 detik)
  - `X-DCISP-Signature: hex(HMAC-SHA256(device_token, METHOD + path + timestamp + rawBody))`
    dengan `device_token` = hash API key terminal yang tersimpan (`devices.api_key_hash`),
    dibandingkan dengan perbandingan waktu-konstan.
  - Endpoint terminal (`terminal-tap`, `sync-offline`) menerima **tanda tangan perangkat
    yang sah ATAU Bearer access token operator**; tanpa keduanya mengembalikan `401`
    sebelum logika bisnis berjalan.
...
### 1.7 Rate Limiting
* `/api/v1/auth/*`: 10 requests / minute (fail-closed bila Redis down).
* General API routes (termasuk `/attendance/terminal-tap`): 120 requests / minute
  per IP / pengguna terautentikasi (fail-open bila Redis down, kecuali scope auth).
...
* **Response (200 OK):** Returns new `TokenPair`; sesi lama dicabut seketika (reuse token lama → `401`).
* **Possible Errors:** `401 Unauthorized` (expired, tampered, revoked, or rotated token).

#### POST /api/v1/auth/logout
* **Purpose:** Revokes the caller device refresh session immediately.
* **Authentication:** Required (`Bearer JWT`)
* **Request Body:** `{ "refresh_token": "string" }`
* **Response (200 OK):** Session revoked; subsequent refresh with the same token → `401`.
...
#### POST /api/v1/people/batches
* **Purpose:** Creates a new intern cohort batch and automatically initializes its `Batch Fund` (BR-021).
* **Authentication:** Required
* **Authorization:** `RequirePermission('people.batches', 'create', 'WORKFORCE_AND_PEOPLE')`
* **Catatan DOC-01:** path aktual adalah `/api/v1/people/batches` (di bawah grup domain people),
  bukan `/api/v1/batches`.
* **Request Body:**
  ```json
  {
...
* **Rules (dievaluasi dinamis dari tabel `permissions`, BR-001 — tanpa hardcode nama peran di kode):**
  - Izin `projects.marketplace:view` (cakupan selain `PUBLIC_DATA`) → `INTERN_ONLY` + `PUBLIC`.
  - Izin `projects.marketplace:view` (hanya cakupan `PUBLIC_DATA`, mis. ALUMNI) → `PUBLIC` saja (BR-003).
  - Izin `projects.marketplace:view_private` (mis. PROJECT_MANAGER, ADMIN) → ditambah `PRIVATE`.
  - Detail proyek `PRIVATE` mensyaratkan keanggotaan tim, kepemilikan, atau izin `view_private`.
  - Peran `SCANNER_OPERATOR` hanya memperoleh direktori `PUBLIC` read-only.
...
#### POST /api/v1/projects/:id/final-contributions
...
* **Catatan DOC-01:** path aktual adalah `/api/v1/projects/:id/final-contributions`,
  bukan `/api/v1/projects/:id/contributions/finalize`.
* **Request Body:**
  ```json
  {
    "contributions": [
...
* **Status: PLANNED (belum diimplementasikan — FR-041 Certificate Engine).**
* **Purpose (rencana):** Public endpoint to verify authenticity of digital graduation scroll.
* **Authentication (rencana):** Public
