...
1. `attendance_event_logs` (Physical & digital presence stream)
2. `financial_ledgers` (Double-entry accounting journal entries)
3. `audit_logs` (Security and administrative change logs)
4. `xp_transactions` (Experience point ledger records)
...
* **`xp_transactions`:** Append-only ledger of experience point mutations partitioned into 3 isolated schemes (`INTERN_XP`, `PROJECT_XP`, `ALUMNI_CONTRIBUTION` - BR-004).
...
| `chk_wallet_balance` | `CHECK` | `current_balance >= 0.00` (No unauthorized overdraft) | `wallets` |
| `chk_schedule_times` | `CHECK` | `start_time < end_time AND break_start < break_end` | `work_schedules` |
| `chk_batch_dates` | `CHECK` | `end_date >= start_date` | `batches` |
| `chk_leave_dates` | `CHECK` | `end_date >= start_date` | `leave_requests` |
| `uq_idempotency_key` | `UNIQUE` | Eliminates duplicate hardware/offline ingestion | `attendance_event_logs` |
