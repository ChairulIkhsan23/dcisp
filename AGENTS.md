...
1. **Read Requirements:** Read the relevant section of `docs/prd/PRD-DCISP-V1.md` and the corresponding BRD in `docs/brd/`.
2. **Read Technical Contracts:** Review `docs/API.md` for endpoint contracts and `docs/DATABASE.md` for schema constraints.
3. **Inspect Existing Implementation:** Read existing files in `backend/` or `frontend/` to understand conventions, imports, and dependencies.
4. **Identify Affected Modules & Scope:** Determine if the task touches Identity, Workforce, Projects, Performance, or Finance.
5. **Check Authorization & Privacy Impact:** Ensure changes adhere to Dynamic RBAC (*BR-001*), Scanner Isolation (*BR-002*), and Privacy-First tracking (*BR-010*).
...
* **Zero Hardcoding on Roles & Policies:** Never check roles with static string equality in application code; always evaluate permissions via RBAC guards (*BR-001*).
